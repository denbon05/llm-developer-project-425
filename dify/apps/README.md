...
| `escalate_stale.yml` | Schedule Trigger (cron every minute) → HTTP `POST http://ticketing:8080/v1/tickets/escalate-stale`. App env `ESCALATION_SECONDS` (30) is sent as JSON `older_than_seconds`. HTTP Request retry: 3 × 100ms. No User Input start; no gateway API key. Cutoff rules stay in ticketing. |
