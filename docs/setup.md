...
2. **Tests** (optional; no Compose, no paid models; fake Dify):
   ...
   This does not prove the live Studio graph.
...
6. **Workflow app.** On a fresh Studio, **Create** → **Import DSL** →
   `dify/apps/email_helpdesk.yml` (the committed no-model graph: MCP tool
   nodes, IF/ELSE, Code/Template stubs, one Variable Aggregator, End
   `reply_text` / `ticket_id`). See
...
`dify/.env` (default `ws://localhost:13080`). Weaviate, Ollama, Dify
Postgres, and Redis have no host ports.
...
only if you need a timer other than 86400. Keep
